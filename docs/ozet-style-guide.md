...
OZET Swift Style Guide
=================

![Swift](https://img.shields.io/badge/Swift-5.4-orange.svg)

구성원들이 Swift 코드를 이해하기 쉽고 명확하게 작성하기 위한 스타일 가이드입니다. 구성원들의 의사결정에 따라 수시로 변경될 수 있습니다.

본 문서는 StyleShare의 [Swift Style Guide](https://github.com/StyleShare/swift-style-guide)를 기반으로 작성되었습니다.
...
- 들여쓰기에는 탭(tab) 대신 **2개의 space**를 사용합니다.
...
- 뷰의 Constraint는 [SnapKit](https://github.com/SnapKit/SnapKit)을 이용해 관리합니다.

- 불필요한 `self`, `inset`, `offset`은 사용하지 않습니다.
...
- 이미지, 스토리보드 등의 리소스는 [R.swift](https://github.com/mac-cain13/R.swift)를 이용해 관리합니다.

- 반복문을 돌려서 이미지를 가져오는 등의 특별한 경우를 제외하곤 UIImage(named:)를 사용하지 않으며, #imageLiteral은 절대 사용하지 않습니다.
  ...
  **나쁜 예**:
  ...
  let image = #imageLiteral(resourceName: "my-image") // my-image가 없을 때 runtime error!!
  ```

- 앱에서 사용하는 Preset 폰트와 색상값은 extension을 이용해 사용합니다.

  - UIColor+Preset.swift
  - UIFont+Growth.swift

  ```swift
  self.label.textColor = .kurly.purple
  self.label.font = .system.regular(14)
  ```
...
- 상수를 정의할 때에는 `enum`를 만들어 비슷한 상수끼리 모아둡니다. 재사용성과 유지보수 측면에서 큰 향상을 가져옵니다. `struct` 대신 `enum`을 사용하는 이유는, 생성자가 제공되지 않는 자료형을 사용하기 위해서입니다. [CGFloatLiteral](https://github.com/devxoul/CGFloatLiteral)을 사용해서 코드를 단순화시킵니다.
    ...
    final class ProfileViewController: UIViewController {
      ...
      private enum Font {
        static let nameLabel = UIFont.system.regular(14)
        static let bioLabel = UIFont.system.bold(12)
      }

      private enum Color {
        static let nameLabelText = UIColor.black
        static let bioLabelText = UIColor.red
      }
...
- 프로토콜을 적용할 때 이외에는 extension을 사용하지 않습니다. 특히, 메서드의 접근 제어<sup>Access Control</sup>를 위해 extension을 분리하지 않습니다.
    ...
    **좋은 예**:
    ...
    final class MyViewController: UIViewController {
      func doSomething() {}
      private func firstPrivateMethod() {}
      private func secondPrivateMethod() {}
    }
    ```
    ...
    **나쁜 예**:
    ...
    private extension MyViewController {
      func firstPrivateMethod() {}
      func secondPrivateMethod() {}
    }
    ```
